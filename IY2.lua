if IY_LOADED and not _G.IY_DEBUG == true then
	...
	warn("Infinite Yield is already running!")
	return
end
...
Title.TextSize = 18
Title.Text = "Infinite Yield [DEVELOPER]"
...
CMDs[#CMDs + 1] = { NAME = "hydroxide", DESC = "Opens Hydroxide Remote" }
CMDs[#CMDs + 1] = { NAME = "cobalt", DESC = "Opens Cobalt Remote" }
CMDs[#CMDs + 1] = { NAME = "sUNC", DESC = "Opens sUNC Test" }
...
function refresh(plr)
	...
	respawn(plr)
	task.spawn(function()
		plr.CharacterAdded:Wait():WaitForChild("HumanoidRootPart").CFrame, workspace.CurrentCamera.CFrame =
			pos, task.wait() and pos1
		refreshCmd = false
	end)
end
...
Players.LocalPlayer.OnTeleport:Connect(function(State)
	if KeepInfYield and not TeleportCheck and queueteleport then
		TeleportCheck = true
		queueteleport("loadstring(game:HttpGet('https://raw.githubusercontent.com/GhostSobe/Script/refs/heads/main/IY2.lua'))()")
	end
end)
			...
			pcall(function()
				speaker.Character.HumanoidRootPart.Velocity = (
					(Humanoid.MoveDirection ~= Vector3.new() or UserInputService:IsKeyDown(Enum.KeyCode.Space))
						and speaker.Character.HumanoidRootPart.Velocity
					or Vector3.new()
				)
			end)
		end)
...
addcmd("offset", {}, function(args, speaker)
	...
	if speaker.Character then
		speaker.Character:TranslateBy(
			Vector3.new(tonumber(args[1]) or 0, tonumber(args[2]) or 0, tonumber(args[3]) or 0)
		)
	end
end)
...
addcmd("explorer", { "dex" }, function(args, speaker)
	notify("Loading", "Hold on a sec")
	loadstring(game:HttpGet("https://androssy.net/files/iy/Dex.lua"))()
end)
...
addcmd("sUNC", {}, function(args, speaker)
	...
	loadstring(game:HttpGet("https://raw.githubusercontent.com/GhostSobe/Script/refs/heads/main/sUNC"))()
end)
...
addcmd("antichatlogs", { "antichatlogger" }, function(args, speaker)
	...
	local MessagePosted, _ = pcall(function()
		rawset(
			require(speaker:FindFirstChild("PlayerScripts"):FindFirstChild("ChatScript").ChatMain),
			"MessagePosted",
			{
				["fire"] = function(msg)
					return msg
...
addcmd("stareat", { "stare" }, function(args, speaker)
	...
	for i, v in pairs(players) do
		...
		if
			not Players.LocalPlayer.Character:FindFirstChild("HumanoidRootPart")
			and Players[v].Character:FindFirstChild("HumanoidRootPart")
		then
			return
		end
...
task.spawn(function()
	...
	if IsOnMobile then
		notify(
			"Unstable Device",
			"On mobile, Infinite Yield may have issues or features that are not functioning correctly."
		)
	end
end)
